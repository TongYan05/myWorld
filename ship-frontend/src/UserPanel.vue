...
const fetchUsers = async () => {
  ...
  try {
    ...
    const url = `/api/userInfo/list?${params.toString()}`
    console.log('🔍 [User] 请求URL:', url)

    const res = await fetch(url)
    console.log('📡 [User] 响应状态:', res.status, res.statusText)

    const data = await res.json()
    console.log('📦 [User] 完整返回数据:', JSON.stringify(data, null, 2))
    console.log('📋 [User] records数组:', Array.isArray(data.records) ? `✅ 长度${data.records.length}` : '❌ 不是数组', data.records)
    console.log('🔢 [User] total값:', data.total, typeof data.total)

    users.value = Array.isArray(data.records) ? data.records : []
    total.value = Number(data.total) || 0

    console.log('✅ [User] 最终users数组长度:', users.value.length)
  } catch (e) {
    console.error('❌ [User] 获取用户数据失败', e)
  } finally {
    loading.value = false
  }
}
...
const openAdd = () => {
  ...
  form.value = { username: '', password: '', name: '', email: '', phone: '', status: 1 }
...
const handleSave = async () => {
  try {
    if (editingUser.value) {
      await fetch('/api/userInfo', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form.value) })
    } else {
      await fetch('/api/userInfo', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form.value) })
    }
...
const handleDelete = async (id) => {
  ...
  try {
    await fetch(`/api/userInfo/${id}`, { method: 'DELETE' })
    fetchUsers()
  } catch (e) {
  ...
  <div class="panel">
    <div class="panel-header">
      <div class="search-box">
        ...
        <input v-model="keyword" @keyup.enter="handleSearch" placeholder="搜索用户名、姓名、邮箱、电话..." />
      ...
      <table class="data-table">
        <thead>
          <tr>
            ...
            <th class="sortable" @click="toggleSort('username')">用户名 <span class="sort-icon">{{ getSortIcon('username') }}</span></th>
            <th class="sortable" @click="toggleSort('name')">姓名 <span class="sort-icon">{{ getSortIcon('name') }}</span></th>
            <th>邮箱</th>
            <th>电话</th>
            <th>状态</th>
            <th class="sortable" @click="toggleSort('loginFailCount')">登录失败 <span class="sort-icon">{{ getSortIcon('loginFailCount') }}</span></th>
            <th>最后登录IP</th>
            <th class="sortable" @click="toggleSort('lastLoginTime')">最后登录 <span class="sort-icon">{{ getSortIcon('lastLoginTime') }}</span></th>
            <th class="sortable" @click="toggleSort('createTime')">创建时间 <span class="sort-icon">{{ getSortIcon('createTime') }}</span></th>
            <th>操作</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="u in users" :key="u.id" class="data-row">
            ...
            <td>{{ u.phone || '-' }}</td>
            <td>
              <span :class="['status-tag', u.status === 1 ? 'active' : 'locked']">
                {{ getStatusText(u.status) }}
              </span>
            </td>
            <td class="id-cell">{{ u.loginFailCount || 0 }}</td>
            <td class="id-cell">{{ u.lastLoginIp || '-' }}</td>
            <td class="time-cell">{{ formatDate(u.lastLoginTime) }}</td>
            <td class="time-cell">{{ formatDate(u.createTime) }}</td>
            <td class="action-cell">
              <button class="act-btn edit" @click="openEdit(u)" title="编辑">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/>
                  <path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/>
                </svg>
              </button>
              <button class="act-btn delete" @click="handleDelete(u.id)" title="删除">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
.status-tag { padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 500; }
.status-tag.active { background: rgba(34, 197, 94, 0.1); border: 1px solid rgba(34, 197, 94, 0.2); color: var(--green); }
.status-tag.locked { background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.2); color: var(--red); }
...
.form-field select:focus { border-color: var(--cyan-dim); }
