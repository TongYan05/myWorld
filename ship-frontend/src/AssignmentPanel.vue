...
const fetchAssignments = async () => {
  ...
  try {
    ...
    if (filterShipId.value) params.set('shipId', filterShipId.value)
    if (filterCrewId.value) params.set('crewId', filterCrewId.value)
    ...
    const url = `/api/shipAssignment/list?${params.toString()}`
    console.log('🔍 [Assignment] 请求URL:', url)

    const res = await fetch(url)
    console.log('📡 [Assignment] 响应状态:', res.status, res.statusText)

    const data = await res.json()
    console.log('📦 [Assignment] 完整返回数据:', JSON.stringify(data, null, 2))
    console.log('📋 [Assignment] records数组:', Array.isArray(data.records) ? `✅ 长度${data.records.length}` : '❌ 不是数组', data.records)
    console.log('🔢 [Assignment] total값:', data.total, typeof data.total)

    assignments.value = Array.isArray(data.records) ? data.records : []
    total.value = Number(data.total) || 0

    console.log('✅ [Assignment] 最终assignments数组长度:', assignments.value.length)
  } catch (e) {
    console.error('❌ [Assignment] 获取任务分配数据失败', e)
  } finally {
    loading.value = false
  }
}
...
const openAdd = () => {
  ...
  form.value = { shipId: null, crewId: null, jobPosition: '', salary: null, boardDate: '', disembarkDate: '' }
...
const handleSave = async () => {
  try {
    if (editingAssignment.value) {
      await fetch('/api/shipAssignment', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form.value) })
    } else {
      await fetch('/api/shipAssignment', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form.value) })
    }
...
const handleDelete = async (id) => {
  if (!confirm('确认删除该任务分配记录？')) return
  try {
    await fetch(`/api/shipAssignment/${id}`, { method: 'DELETE' })
    fetchAssignments()
  } catch (e) {
  ...
  <div class="panel">
    <div class="panel-header">
      <div class="search-box">
        ...
        <input v-model="keyword" @keyup.enter="handleSearch" placeholder="搜索船舶ID、船员ID、职务..." />
    ...
    <div class="filter-bar" v-show="showFilter">
      ...
      <div class="filter-group">
        <label>船舶ID</label>
        <input v-model="filterShipId" type="number" placeholder="输入船舶ID" />
      </div>
      <div class="filter-group">
        <label>船员ID</label>
        <input v-model="filterCrewId" type="number" placeholder="输入船员ID" />
      </div>
      <div class="filter-group">
        <label>上船日期从</label>
    ...
    <div class="table-wrapper">
      <table class="data-table">
        <thead>
          <tr>
            <th class="sortable" @click="toggleSort('assignmentId')">ID <span class="sort-icon">{{ getSortIcon('assignmentId') }}</span></th>
            <th class="sortable" @click="toggleSort('shipId')">船舶ID <span class="sort-icon">{{ getSortIcon('shipId') }}</span></th>
            <th class="sortable" @click="toggleSort('crewId')">船员ID <span class="sort-icon">{{ getSortIcon('crewId') }}</span></th>
            <th class="sortable" @click="toggleSort('jobPosition')">职务 <span class="sort-icon">{{ getSortIcon('jobPosition') }}</span></th>
            <th class="sortable" @click="toggleSort('salary')">薪资 <span class="sort-icon">{{ getSortIcon('salary') }}</span></th>
            <th class="sortable" @click="toggleSort('boardDate')">上船日期 <span class="sort-icon">{{ getSortIcon('boardDate') }}</span></th>
            <th class="sortable" @click="toggleSort('disembarkDate')">离船日期 <span class="sort-icon">{{ getSortIcon('disembarkDate') }}</span></th>
            <th>操作</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="a in assignments" :key="a.assignmentId" class="data-row">
            <td class="id-cell">{{ a.assignmentId }}</td>
            <td><code class="ref-code">{{ a.shipId }}</code></td>
            <td><code class="ref-code">{{ a.crewId }}</code></td>
            <td><span class="tag">{{ a.jobPosition }}</span></td>
            <td class="salary-cell">{{ a.salary ? '¥' + Number(a.salary).toLocaleString() : '-' }}</td>
            <td class="time-cell">{{ formatDate(a.boardDate) }}</td>
            <td class="time-cell">{{ formatDate(a.disembarkDate) }}</td>
            <td class="action-cell">
              <button class="act-btn edit" @click="openEdit(a)" title="编辑">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/>
                  <path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/>
                </svg>
              </button>
              <button class="act-btn delete" @click="handleDelete(a.assignmentId)" title="删除">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <div v-if="showModal" class="modal-overlay" @click.self="showModal = false">
        <div class="modal-card">
          <div class="modal-header">
            <h3>{{ editingAssignment ? '编辑分配' : '新增分配' }}</h3>
          ...
          <div class="modal-body">
            <div class="form-grid">
              ...
              <div class="form-field">
                <label>船员ID</label>
                <input v-model.number="form.crewId" type="number" placeholder="请输入船员ID" />
              </div>
              <div class="form-field">
                <label>职务</label>
                <select v-model="form.jobPosition">
              ...
              <div class="form-field">
                <label>薪资</label>
                <input v-model="form.salary" type="number" step="0.01" placeholder="请输入薪资" />
              </div>
              <div class="form-field">
                <label>上船日期</label>
                <input v-model="form.boardDate" type="datetime-local" />
              </div>
              <div class="form-field">
                <label>离船日期</label>
                <input v-model="form.disembarkDate" type="datetime-local" />
              </div>
            </div>
          </div>
